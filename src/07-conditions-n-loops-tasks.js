...
function isCreditCardNumber(cnn) {
  ...
  arr.pop();
  const newArr = arr.reverse().map((item, index) => {
    let newItem;
    if (index % 2 === 0) {
      newItem = item * 2;
      if (newItem > 9) {
        newItem = newItem.toString().split('').reduce((sum, current) => sum + +current, 0);
      }
    } else {
      newItem = +item;
    }
  ...
  const sumDigit = newArr.reduce((sum, current) => sum + current, 0);
  if (lastDigit === 10 - (sumDigit % 10)) return true;
  return false;
}
