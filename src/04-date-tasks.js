...
function timeSpanToString(startDate, endDate) {
  let hh = (endDate.getHours() - startDate.getHours()).toString();
  if (hh.length === 1) {
    hh = `0${hh}`;
  }
  let mm = (endDate.getMinutes() - startDate.getMinutes()).toString();
  if (mm.length === 1) {
    mm = `0${mm}`;
  }
...
function angleBetweenClockHands(date) {
  ...
  let hoursDegAngle = 0.5 * (h * 60 + m);
  if (hoursDegAngle > 360) {
    hoursDegAngle -= 360;
  }
  const minutesDegAngle = 6 * m;
  let angleDeg = Math.abs(hoursDegAngle - minutesDegAngle);
  if (angleDeg > 180) {
